...
while 1:
    ...
    for radiator in addresses_to_poll:
        ...
        target_temperature = target_temperature + ((now.hour % 2)*0.5)
        ret = client1.publish(topic+"/set/" + str(radiator) + "/1/SET_TEMPERATURE", target_temperature)                   #publish
